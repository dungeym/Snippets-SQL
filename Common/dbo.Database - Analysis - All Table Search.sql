--=================================================================================================
-- Search every column of type: text, ntext, varchar, char, nvarchar, sysname, nchar
-- in every table (named like 'tbl%') for the defined text.
--=================================================================================================
...
BEGIN
	...
	DECLARE @KeyColumn VARCHAR(255) = (	
										...
										(
											...
											INNER JOIN sys.columns c WITH (NOLOCK) ON c.column_id = ic.column_id AND c.[object_id] = i.[object_id]
											AND i.[type] = 1 
	...
	SELECT 
		...
		,[TSQL] = 
			...
			END 
			+ '' + QUOTENAME(@Table) + ' WITH (NOLOCK) WHERE ' + QUOTENAME(c.[name]) + ' IS NOT NULL AND ' + QUOTENAME(c.[name]) + ' LIKE ''' + @LikePattern + ''''
	FROM sys.columns c WITH (NOLOCK)
...
-- SELECT * FROM sys.types t WITH (NOLOCK) WHERE t.system_type_id IN (35,99,167,175,231,239) ORDER BY t.system_type_id
-- SELECT * FROM #Table
