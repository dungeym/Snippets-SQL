...
(
	...
	IsIdentity			BIT,
	IsFkColumn			BIT,
	CurrentMaxValue		BIGINT -- Sybase is unhappy with SQL_VARIANT
)
...
INNER JOIN sys.types y WITH (NOLOCK) ON y.system_type_id = c.system_type_id
LEFT JOIN sys.foreign_key_columns fk WITH (NOLOCK) ON fk.parent_object_id = t.[object_id] AND fk.parent_column_id = c.column_id
WHERE 1=1
AND UPPER(y.name) IN ('INT', 'BIGINT')
AND UPPER(c.name) NOT IN ('CreatedByID', 'UpdatedByID') -- Skip these for PCM
...
BEGIN
	...
	SET @TSQL = 'UPDATE #Data SET CurrentMaxValue = (SELECT MAX(' + @ColumnName + ') FROM ' + @TableName + ' WITH (NOLOCK) WHERE ' + @ColumnName + ' IS NOT NULL) WHERE TableName = ''' + @TableName + ''' AND ColumnName = ''' + @ColumnName + ''''
	--PRINT @TSQL
