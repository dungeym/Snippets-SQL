...
LEFT JOIN sys.default_constraints dc WITH (NOLOCK) ON dc.[object_id] = c.default_object_id AND dc.parent_object_id = o.[object_id] AND dc.parent_column_id = c.column_id
LEFT JOIN sys.foreign_key_columns fkc WITH (NOLOCK) ON fkc.parent_object_id = o.[object_id] AND fkc.parent_column_id = c.column_id
LEFT JOIN 
(
	SELECT 
		o.[object_id]
