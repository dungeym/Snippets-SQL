...
OPEN Table_Cursor;
FETCH NEXT FROM Table_Cursor INTO @TSQL;
...
SELECT 'GRANT SELECT ON [' + name + '] TO [' + @Name + ']' 
FROM sys.views 
ORDER BY [name] ASC;


OPEN View_Cursor;
FETCH NEXT FROM View_Cursor INTO @TSQL;
...
SELECT 'GRANT EXECUTE ON [' + name + '] TO [' + @Name + ']' 
FROM sys.procedures 
ORDER BY [name] ASC;


OPEN Procedure_Cursor;
FETCH NEXT FROM Procedure_Cursor INTO @TSQL;
...
FROM sys.objects 
WHERE 1=1
AND [type] IN ('FN', 'TF', 'FS', 'FT') 
ORDER BY name ASC;


OPEN Function_Cursor;
FETCH NEXT FROM Function_Cursor INTO @TSQL;
