...
BEGIN
	...
	SELECT 
		...
		(
			SELECT 
				c.name + ',' AS [text()]
 FROM sys.columns c 
 INNER JOIN sys.stats_columns sc ON sc.column_id = c.column_id AND sc.[object_id] = c.[object_id] AND sc.stats_id = st.stats_id
 WHERE 1=1
 ...
 FOR XML PATH ('')
	...
	LEFT JOIN sys.tables t ON st.[object_id] = t.[object_id]
	WHERE 1=1
	AND t.[type] = 'U'
...
*/
	...
	INNER JOIN sys.dm_db_partition_stats st ON st.[object_id] = i.[object_id] AND st.index_id = i.index_id
