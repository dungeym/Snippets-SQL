...
findfk: ;WITH cte AS (SELECT ObjectId = CONVERT(INT, OBJECT_ID('%$CURSOR$%'))) SELECT TableName = OBJECT_NAME(k.parent_object_id) + '.' + c1.name, HasReferenceTo = OBJECT_NAME(k.referenced_object_id) + '.' + c2.name, ConstraintName = k.name FROM sys.foreign_keys k INNER JOIN cte ON (cte.ObjectId = k.parent_object_id OR cte.ObjectId = k.referenced_object_id) INNER JOIN sys.foreign_key_columns kc ON kc.parent_object_id = k.parent_object_id AND kc.referenced_object_id = k.referenced_object_id INNER JOIN sys.columns c1 ON c1.object_id = kc.parent_object_id AND c1.column_id = kc.parent_column_id INNER JOIN sys.columns c2 ON c2.object_id = kc.referenced_object_id AND c2.column_id = kc.referenced_column_id ORDER BY OBJECT_NAME(k.parent_object_id) ASC , OBJECT_NAME(k.referenced_object_id) ASC 
findsp: SELECT * FROM sys.procedures WHERE [name] LIKE '%$CURSOR$%' ORDER BY OBJECT_NAME(object_id) ASC, [name] ASC
findtable: SELECT * FROM sys.tables WHERE [name] LIKE '%$CURSOR$%' ORDER BY [name] ASC
